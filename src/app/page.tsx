...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-6">
      {/* Hero */}
      <div className="rounded-2xl bg-gradient-to-br from-navy-900 via-navy-700 to-brand-400 p-8 md:p-10 text-white relative overflow-hidden">
        ...
        <div className="relative">
          <span className="inline-block text-xs font-semibold uppercase tracking-widest text-brand-200 mb-3">
            MedFaktor Clinic · Rzeszów
          </span>
          <h1 className="text-2xl md:text-3xl font-bold leading-snug mb-3">
            Medycyna Pracy
          </h1>
          <p className="text-brand-100 text-sm md:text-base leading-relaxed max-w-lg">
          ...
          <div className="grid gap-2">
            {services.map((s: {_id: string; title: string; slug: {current: string}}) => (
              <Link
                ...
                className="group flex items-center justify-between bg-white border border-slate-100 rounded-xl px-5 py-4 hover:border-brand-200 hover:shadow-sm hover:shadow-brand-100 transition-all"
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-slate-800 group-hover:text-navy-900 transition-colors">
                    {s.title}
                  </span>
                </div>
                <svg className="w-4 h-4 text-slate-300 group-hover:text-brand-400 group-hover:translate-x-0.5 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
